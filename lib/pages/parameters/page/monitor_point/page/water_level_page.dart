...
class _WaterlevelPageState extends State<WaterlevelPage> {
  ...
  int levelIndex = Get.arguments['levelIndex'];
  ...
  void initData() async {
    ...
    String string = '';
    levelIndex == 1
        ? string = DataTool.sendGetMessageData('0$monitorPoint', '50', 36)
        : string = DataTool.sendGetMessageData('0$monitorPoint', '70', 36);
    print('string:$string');
            ...
            children: [
              Expanded(
                child: GetBuilder<MonitorController>(builder: (monitorController) {
                  ...
                  if(levelIndex == 1) {
                    if(monitorController.waterLevelList.length == 0) {
                      return DefaultNoDataViewWidget();
                    }
                  } else {
                    if(monitorController.waterLevelTwoList.length == 0) {
                      return DefaultNoDataViewWidget();
                    }
                  }

                  List dataCurrentList = [];
                  levelIndex == 1 ? dataCurrentList.addAll(monitorController.waterLevelList):dataCurrentList.addAll(monitorController.waterLevelTwoList);

                  for (int i = 0; i < dataCurrentList.length; i++) {
                    List data = dataCurrentList[i];
                    RegisterData registerData = data.first;
                    List<int> list = data.last;
                    String str = intListToDisplayString(list);
                    if(allData.length  < 7 && monitorController.funcType == 3 && !registerData.isOnlyRead) {
                      allData.add(str);
                    }
                  }

                  return ListView.builder(
                    itemCount: dataCurrentList.length,
                    itemBuilder: (BuildContext context, int index) {
                      List itemData = dataCurrentList[index];
                      ...
                      if(index == 0) {
                        ...
                        if(ByteTool.codeToInt(hexList) == 6) {
                          content = '未启用';
                        }
                      } else if (index == 4) {
                        ...
                        /// 获取类型
                        List typeData = dataCurrentList[4];
                      ...
                      return InkWell(
                        ...
                        onTap: () {
                          print(index);
                          List itemData = dataCurrentList[index];
                          RegisterData registerDataTap = itemData.first;
                          if(registerDataTap.isOnlyRead) return;
                          RegisterData newReg = RegisterData(
                            registerAddressHigh: registerDataTap.registerAddressHigh,
                            registerAddressLow: registerDataTap.registerAddressLow,
                            content: registerDataTap.content,
                            addressHigh: registerDataTap.addressHigh,
                            addressLow: registerDataTap.addressLow,
                            isOnlyRead: registerDataTap.isOnlyRead,
                            isASCII: registerDataTap.isASCII,
                            unit: registerDataTap.unit,
                            length: registerDataTap.length,
                            multiple: registerDataTap.multiple,
                            instructions: registerDataTap.instructions,
                            value: registerDataTap.value,
                            defaultString: registerDataTap.defaultString,
                            minValue: registerDataTap.minValue,
                            maxValue: registerDataTap.maxValue,
                            isIpAddress: registerDataTap.isIpAddress,
                            isMinus: registerDataTap.isMinus,
                          );
                          print('registerData:${registerDataTap.addressLow}');
                          newReg.addressHigh = '0$monitorPoint';
                          if(levelIndex == 2) {
                            /// 点数1和点数2相差32
                            List<int> bytes = HEX.decode(newReg.addressLow);
                            print('bytes:$bytes');
                            String string = '';
                            int count = bytes.first + 32;
                            string = count.toRadixString(16);

                            newReg.addressLow = string;
                            print('string:$string');
                          }
                          if (newReg.isOnlyRead) return;
                          if (index == 4 || index == 0) {
                            print(newReg.addressLow);
                            wateLevelTypeChanged(index, newReg, index);
                          } else {
                            /// 获取类型
                            List typeData = dataCurrentList[4];
                            RegisterData typeRegisterData = typeData.first;
                            ...
                            if(index == 3) {
                              if (typeIndex == 0) {
                                newReg.defaultString = '只能设置0';
                                newReg.isIpAddress = false;
                                newReg.minValue = 0;
                                newReg.maxValue = 0;
                              } else {
                                newReg.defaultString = '1~255';
                                newReg.isIpAddress = false;
                                newReg.minValue = 1;
                                newReg.maxValue = 255;
                              }
                              ...
                              if (typeIndex == 0 || typeIndex == 1 || typeIndex == 3 || typeIndex == 4) {
                                newReg.defaultString = '0~1';
                                newReg.minValue = 0;
                                newReg.maxValue = 1;
                              } else {
                                newReg.defaultString = '0～65535';
                                newReg.minValue = 0;
                                newReg.maxValue = 65535;
                              }
                            }
                            funcSendMonitor(newReg, index);
                          }
                        },
                      );
                    },
              ...
              RxdWidget(
                xiafaFuncation: (){
                  ...
                  String addLow = levelIndex == 1? '50':'70';
                  String headerStr = DataTool.sendWriteMessageMultipleData('0$monitorPoint', addLow, 28);
                  print(headerStr);
  ...
  void wateLevelTypeChanged(int type, RegisterData registerData, int indexT) {
    // 0：FMD55_AI
    // 1:ECU500_AI
    // 2:规约1
    // 3:规约2
    ...
    var list;
    type == 0
        ? list = ['数据点号1', '数据点号2', '数据点号3', '数据点号4', '数据点号5', '数据点号6','未启用']
