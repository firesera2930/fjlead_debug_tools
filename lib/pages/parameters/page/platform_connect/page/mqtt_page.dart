...
class _MQTTPageState extends State<MQTTPage> {
            ...
            children: [
              Expanded(
                child: GetBuilder<MonitorController>(builder: (monitorController) {
                  if(monitorController.mqttDataList.length == 0) {
                    return DefaultNoDataViewWidget();
                  }
                  return ListView.builder(
                    itemCount: monitorController.mqttDataList.length,
                    itemBuilder: (BuildContext context, int index) {
                      List itemData = monitorController.mqttDataList[index];
                      ...
                      return InkWell(
                        ...
                        onTap: () {
                          List itemData = monitorController.mqttDataList[index];
                          RegisterData registerData = itemData.first;
                          funcSendMQTTMonitor(registerData, index);
                        },
                      );
                    },
                  );
              ...
              RxdWidget(
                xiafaFuncation: (){
                  ...
                  int indexM = allMap['index'];
                  String valueM = allMap['value'];
                  List itemData = monitorController.mqttDataList[indexM];
                  RegisterData registerData = itemData.first;
  ...
  void funcSendMQTTMonitor(RegisterData registerData, int index) {
    showDialog(
